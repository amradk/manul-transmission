...
vms = [
  {:v_name => 'fbsd', :v_mem => 1024, :v_cpu => 1, :v_ip => '192.168.20.10'},
]
...
Vagrant.configure("2") do |config|
  ...
  config.vm.box_version = "2021.04.15"
  #config.ssh.shell = "sh"
    ...
    config.vm.define i[:v_name] do |machine|
      ...
      machine.vm.provision :ansible do |ansible|
        ansible.raw_arguments = [
          "--connection=paramiko",
          "--private-key=./.vagrant/machines/fbsd/virtualbox/private_key"
        ]
        ansible.inventory_path = "./tests/test-inventory"
